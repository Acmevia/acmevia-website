...
function Frame({
  ...
  return (
    <div aria-hidden="true" className="relative">
      <div className="overflow-hidden rounded-lg border border-paper/15 bg-surface shadow-[0_24px_80px_rgba(0,178,255,0.08)]">
        {/* Browser chrome */}
        <div className="flex items-center gap-3 border-b border-paper/10 px-4 py-2.5">
          ...
          <span className="rounded-full bg-paper/5 px-3 py-1 font-mono text-[0.625rem] tracking-wide text-paper/50">
            {url}
          </span>
        </div>
...
const Bar = ({ w, className = "bg-paper/15" }: { w: string; className?: string }) => (
  ...
  return (
    <Frame url="lms.yourbrand.com/admin">
      <div className="flex h-full gap-3">
        {/* Sidebar */}
        <div className="hidden w-1/5 flex-col gap-3 rounded border border-paper/10 p-3 sm:flex">
        ...
        <div className="flex flex-1 flex-col gap-3">
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={l} className="rounded border border-paper/10 p-2.5">
          ...
          <div className="flex-1 rounded border border-paper/10 p-3">
            <p className="font-mono text-[0.5rem] tracking-[0.14em] text-azure-text">EXAM RESULTS — AUTO-GRADED</p>
            <div className="mt-2.5 space-y-2">
              {["82%", "74%", "91%", "68%"].map((p, i) => (
                <div key={i} className="flex items-center gap-2">
                  <Bar w="26%" />
                  <div className="h-1.5 flex-1 rounded-full bg-paper/5">
    ...
    <Frame url="spaces.yourbrand.com/calendar">
      <div className="flex h-full flex-col gap-2.5">
        <div className="flex items-center justify-between">
          <p className="font-mono text-[0.55rem] tracking-[0.14em] text-azure-text">
            ALL VENUES — WEEK 41
          </p>
          <div className="flex gap-2">
            <span className="rounded-full border border-azure/50 px-2 py-0.5 font-mono text-[0.5rem] text-azure-text">
              0 CONFLICTS
            </span>
          </div>
        </div>
    ...
    <Frame
      ...
      overlay={
        ...
        <div className="absolute -bottom-8 right-4 w-24 rounded-xl border border-azure/40 bg-ink p-2.5 shadow-[0_16px_48px_rgba(0,0,0,0.6)] sm:w-28">
          <p className="font-mono text-[0.5rem] tracking-[0.14em] text-azure-text">POS — RT-14</p>
      ...
      <div className="flex h-full gap-3">
        <div className="flex flex-1 flex-col gap-2">
          ...
          {[
            ...
            <div
              key={i}
              className="flex items-center justify-between rounded border border-paper/10 px-2 py-1.5 font-mono text-[0.5rem] text-paper/60"
            >
              <span className="text-paper/35">{t}</span>
              <span>{r}</span>
              <span className="text-azure-text">{v}</span>
            </div>
          ))}
        </div>
        <div className="hidden w-2/5 flex-col gap-3 sm:flex">
          <div className="rounded border border-paper/10 p-2.5">
          ...
          <div className="relative flex-1 rounded border border-paper/10">
            {/* Agent positions */}
            {[
